...
import { getRound1Status, startRound1 } from "../../api/round1Api";
import { getApiErrorMessage } from "../../api/httpClient";
import { ROUND_CONFIG } from "../../data/roundConfig";

/* ─── RULES ─── */
const RULES = [
  "Submit answers only once. Final submission cannot be edited.",
  "Do not refresh or open a new tab during the active attempt.",
  "No external communication tools or AI assistants are allowed.",
...
function SkeletonLoader() {
  return (
    <motion.div
      ...
      style={{ ...cardBase, padding: "32px", display: "flex", flexDirection: "column", gap: "16px" }}
    >
      {/* Top shimmer line */}
      ...
      {[40, 60, 90, 70, 50, 80].map((w, i) => (
        <motion.div key={i}
          ...
          style={{ height: i === 0 ? "10px" : "14px", borderRadius: "7px",
...
function ConfirmModal({ durationLabel, onConfirm, onCancel, loading }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      style={{ position: "fixed", inset: 0, zIndex: 150, display: "flex", alignItems: "center",
    ...
    >
      <motion.div initial={{ scale: 0.94, y: 16 }} animate={{ scale: 1, y: 0 }}
        ...
        style={{ ...cardBase, width: "100%", maxWidth: "420px", overflow: "hidden",
          boxShadow: "0 40px 100px rgba(0,0,0,0.8)", border: "1px solid rgba(251,146,60,0.28)", position: "relative" }}
      >
        {/* Top bar */}
        <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "2px",
        ...
        <div style={{ padding: "20px 22px 16px", borderBottom: "1px solid rgba(255,255,255,0.07)",
          ...
          <p style={{ ...lbl, color: "rgba(251,146,60,0.65)", marginBottom: "5px" }}>Round 1 · Confirmation</p>
          <h3 style={{ ...mono, fontSize: "15px", fontWeight: 700, color: "#fed7aa", margin: 0 }}>
            Ready to begin?
          </h3>
        </div>
        ...
        <div style={{ padding: "20px 22px" }}>
          ...
          <div style={{ display: "flex", gap: "12px", padding: "14px", borderRadius: "10px",
            background: "rgba(251,146,60,0.07)", border: "1px solid rgba(251,146,60,0.2)", marginBottom: "14px" }}>
            <motion.span animate={{ opacity: [1, 0.3, 1] }} transition={{ duration: 1.4, repeat: Infinity }}
              style={{ width: "7px", height: "7px", borderRadius: "50%", background: "#fb923c",
                boxShadow: "0 0 8px rgba(251,146,60,0.9)", flexShrink: 0, marginTop: "4px" }}/>
            <div>
              <p style={{ ...mono, fontSize: "11px", fontWeight: 700, color: "#fdba74", marginBottom: "4px" }}>
                Timer starts immediately
              </p>
              <p style={{ fontSize: "12px", lineHeight: 1.65, color: "rgba(203,213,225,0.6)",
                fontFamily: "'Inter',sans-serif", margin: 0 }}>
                Once you click <strong style={{ color: "#fb923c" }}>Start Timer</strong>, the{" "}
                <strong style={{ color: "#fdba74" }}>{durationLabel}</strong> countdown begins and cannot be paused.
        ...
        <div style={{ padding: "14px 22px", borderTop: "1px solid rgba(255,255,255,0.07)",
          ...
          <motion.button whileTap={{ scale: 0.97 }} onClick={onConfirm} disabled={loading}
            style={{
              ...
              border: "1px solid rgba(251,146,60,0.38)", background: "rgba(251,146,60,0.14)",
              color: "#fdba74", transition: "all 0.14s",
              display: "flex", alignItems: "center", gap: "7px",
              opacity: loading ? 0.7 : 1,
            }}
            onMouseEnter={e => { if (!loading) { e.currentTarget.style.background = "rgba(251,146,60,0.24)"; e.currentTarget.style.borderColor = "rgba(251,146,60,0.6)"; } }}
            onMouseLeave={e => { e.currentTarget.style.background = "rgba(251,146,60,0.14)"; e.currentTarget.style.borderColor = "rgba(251,146,60,0.38)"; }}
          >
            {loading ? (
...
/* ══════════════════════════════════════════
   MAIN PAGE
══════════════════════════════════════════ */
export default function Round1TermsPage() {
  ...
  const durationLabel = useMemo(
    () => `${Math.floor(config.durationSeconds / 60)} minutes`,
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setError(getApiErrorMessage(err, "Unable to load round status."));
      } finally {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Failed to start Round 1."));
      ...
      >
        ...
        <div style={{ ...cardBase, padding: "26px 28px", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "2px",
            background: "linear-gradient(90deg,transparent,rgba(251,146,60,0.55),transparent)" }}/>
          {/* Ambient glow */}
          <motion.div animate={{ opacity: [0.2, 0.4, 0.2], scale: [1, 1.08, 1] }}
            transition={{ duration: 4, repeat: Infinity }}
            style={{ position: "absolute", top: "-50px", right: "-50px", width: "200px", height: "200px",
              borderRadius: "50%", background: "rgba(251,146,60,0.08)", filter: "blur(50px)", pointerEvents: "none" }}/>

          <div style={{ position: "relative", zIndex: 1 }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: "12px" }}>
              <div>
                <p style={{ ...lbl, color: "rgba(251,146,60,0.65)" }}>{config.title}</p>
                <h2 style={{ fontSize: "20px", fontWeight: 700, color: "#f1f5f9", marginTop: "8px", letterSpacing: "-0.01em" }}>
                  Terms &amp; Conditions
                </h2>
              </div>
              {/* Duration badge */}
              <div style={{ display: "flex", alignItems: "center", gap: "8px", padding: "7px 14px",
                borderRadius: "999px", background: "rgba(251,146,60,0.08)", border: "1px solid rgba(251,146,60,0.22)" }}>
                <span style={{ ...lbl, fontSize: "8px", color: "rgba(251,146,60,0.6)" }}>Duration</span>
                <span style={{ ...mono, fontSize: "12px", fontWeight: 700, color: "#fb923c" }}>{durationLabel}</span>
              </div>
            </div>
            <p style={{ fontSize: "13px", lineHeight: 1.75, color: "rgba(148,163,184,0.55)", marginTop: "10px",
              fontFamily: "'Inter',sans-serif" }}>
              Timer starts immediately after you begin. Read all rules carefully before proceeding.
            </p>
          </div>
        </div>
        ...
        <div style={{ ...cardBase, overflow: "hidden", position: "relative" }}>
          ...
          <div style={{ padding: "14px 20px", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
            <p style={{ ...lbl, color: "rgba(56,189,248,0.55)" }}>◈ Arena Protocol</p>
          </div>
            ...
            {RULES.map((rule, i) => (
              <motion.div key={i}
                ...
                style={{
                  ...
                  background: ruleExpanded === i ? "rgba(251,146,60,0.07)" : "rgba(255,255,255,0.02)",
                  border: ruleExpanded === i ? "1px solid rgba(251,146,60,0.25)" : "1px solid rgba(255,255,255,0.05)",
                }}
              >
                <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                  ...
                  <div style={{
                    ...
                    background: ruleExpanded === i ? "rgba(251,146,60,0.15)" : "rgba(255,255,255,0.04)",
                    border: ruleExpanded === i ? "1px solid rgba(251,146,60,0.3)" : "1px solid rgba(255,255,255,0.07)",
                    color: ruleExpanded === i ? "#fb923c" : "rgba(148,163,184,0.45)",
                ...
                <AnimatePresence>
                  {ruleExpanded === i && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
                      ...
                      <div style={{ marginTop: "10px", marginLeft: "38px", padding: "8px 12px",
                        borderRadius: "7px", background: "rgba(248,113,113,0.06)",
                        border: "1px solid rgba(248,113,113,0.15)" }}>
                        <p style={{ ...mono, fontSize: "10px", letterSpacing: "0.08em",
        ...
        <div style={{ ...cardBase, padding: "20px 22px", display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          <label style={{ display: "flex", alignItems: "flex-start", gap: "12px", cursor: "pointer" }}>
            <div onClick={() => setAgreed(p => !p)} style={{
              ...
              border: agreed ? "1px solid rgba(56,189,248,0.5)" : "1px solid rgba(255,255,255,0.18)",
              background: agreed ? "rgba(56,189,248,0.18)" : "transparent",
              display: "flex", alignItems: "center", justifyContent: "center",
            ...
            <span style={{ fontSize: "12.5px", color: "rgba(203,213,225,0.72)", lineHeight: 1.6 }}>
              I have read and agree to the Round 1 rules and understand the timer starts immediately upon confirmation.
            </span>
          </label>
          ...
          <AnimatePresence>
            {error && (
              <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                style={{ padding: "10px 14px", borderRadius: "9px", display: "flex", alignItems: "center", gap: "10px",
                ...
                <button onClick={() => setError("")} style={{ background: "none", border: "none",
                  color: "rgba(148,163,184,0.45)", cursor: "pointer", fontSize: "14px", flexShrink: 0 }}>✕</button>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
            <motion.button type="button" whileTap={{ scale: 0.97 }}
              ...
              style={{
                ...
                border: agreed ? "1px solid rgba(56,189,248,0.35)" : "1px solid rgba(255,255,255,0.07)",
                background: agreed ? "rgba(56,189,248,0.12)" : "rgba(255,255,255,0.03)",
                color: agreed ? "#7dd3fc" : "rgba(255,255,255,0.22)",
                transition: "all 0.15s", opacity: agreed ? 1 : 0.5, ...mono,
                display: "flex", alignItems: "center", gap: "8px",
              }}
              onMouseEnter={e => { if (agreed) { e.currentTarget.style.background = "rgba(56,189,248,0.2)"; e.currentTarget.style.boxShadow = "0 0 18px rgba(56,189,248,0.2)"; } }}
              onMouseLeave={e => { e.currentTarget.style.background = agreed ? "rgba(56,189,248,0.12)" : "rgba(255,255,255,0.03)"; e.currentTarget.style.boxShadow = "none"; }}
            >
              Initialize Round 1 →
            </motion.button>
